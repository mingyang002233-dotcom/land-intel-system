...
| 會進入實價提醒 | 不會進入實價提醒 |
|--------------|----------------|
| 實價登錄比對命中的買賣交易 | 增貸、抵押設定、他項權利設定 |
| 地主名單疑似變動（未售出） | 繼承、贈與（實價登錄無對應）|
| 已售地主再次命中（確認異動）| 地目調整、分割、合併 |

**非買賣異動的正確歸屬：**
- 增貸 / 抵押 / 他項設定 → 未來「他項權利情報」或「地主金融壓力」模組（尚未建置）
- 不得混入實價提醒報表

**mark_realprice_processed() 觸發條件（三者同時成立）：**
1. `reg_reason == '買賣'`（電傳確認買賣事件）
2. 成功寫入 SQLite land_master
3. 實價提醒報表有該地號的未處理提醒（建議動作不含「已調閱」）

**不觸發情境：** reg_reason 為增貸、抵押、他項、地目、繼承、分割等，即使電傳解析成功，也不標記實價提醒。
