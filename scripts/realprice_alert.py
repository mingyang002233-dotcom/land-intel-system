...
執行：
  ...
  python3 scripts/realprice_alert.py --reconcile ← 回溯比對：將現有報表中已反映的地號標為已調閱
...
def reconcile_realprice_alerts(dry_run: bool = False) -> dict:
    ...
    ══ 核心判定規則（與 is_reflected_in_master 完全一致）══
    「已反映」= 同地號在 DB 已有 reg_reason='買賣' 且 reg_date >= 實價日期（差距 ≤ 90 天）的新地主。
    → 代表「新地主結構已存在於 MASTER」，此實價提醒無需繼續待調閱。

    ══ 明確不使用的欄位 ══
    is_sold / 已售出 → 不作為「已反映」的判定依據。
    「舊地主是否標已售」與「新地主是否已存在」是兩件獨立的事。
    舊 MASTER 清冊歷史資料很多缺乏完整已售標記，不能以此為門檻。
